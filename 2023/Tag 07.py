...
def process_hand(hand, use_variant):
    ...
    if use_variant:
        # Apply specific rules for the variant
    ...
    # Determine the hand strength based on card counts
...
def main():
    ...
    for use_variant in [False, True]:
        ...
        # Sort hands based on strength using the process_hand function
        sorted_hands = sorted(hands_and_bids, key=lambda hb: process_hand(hb[0], use_variant))

        total_score = 0

        # Calculate total score for the case
